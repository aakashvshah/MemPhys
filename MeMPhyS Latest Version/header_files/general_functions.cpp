...
void write_csv(vector<double> &xyz, vector<bool> &boundary_flag, int dim, vector<string> &variable_names, vector<Eigen::VectorXd *> &variable_pointers, const char *file_name)
{
    int nv = xyz.size() / dim;
    FILE *file;
    file = fopen(file_name, "w");
    ...
    for (auto var : variable_names)
        fprintf(file, ",\"%s\"", var.c_str());
    fprintf(file, "\n");
    for (int iv = 0; iv < nv; iv++)
    {
        fprintf(file, "%i,", iv);
        for (int i1 = 0; i1 < dim; i1++) // write x, y, z
            fprintf(file, "%g,", xyz[dim * iv + i1]);
        fprintf(file, "%i,", (int)(boundary_flag[iv]));
        for (auto ptr : variable_pointers) // write variables
            fprintf(file, "%g,", (*ptr)[iv]);
        fprintf(file, "\n");
    }
    fclose(file);
}
...
void write_csv(Eigen::MatrixXd &x_3d, Eigen::MatrixXd &y_3d, Eigen::MatrixXd &z_3d, vector<bool> &boundary_flag, vector<string> &variable_names, vector<Eigen::MatrixXd *> &variable_pointers, const char *file_name)
{
    ...
    for (auto var : variable_names)
        fprintf(file, ",\"%s\"", var.c_str());
    fprintf(file, "\n");
    int nv = x_3d.rows(), nw = x_3d.cols();
    for (int iw = 0; iw < nw; iw++)
        for (int iv = 0; iv < nv; iv++)
        {
            fprintf(file, "%i,", iv);
            fprintf(file, "%g,%g,%g,", x_3d(iv, iw), y_3d(iv, iw), z_3d(iv, iw)); // write x, y, z
            fprintf(file, "%i,", (int)(boundary_flag[iv]));
            for (auto ptr : variable_pointers) // write variables
                fprintf(file, "%g,", (*ptr)(iv, iw));
            fprintf(file, "\n");
        }
    fclose(file);
}
...
{
    for (int iv = 0; iv < phi.rows(); iv++)
        fft.fwd(eigen_temp_vec_complex, phi.row(iv)), phi_fft.row(iv) = eigen_temp_vec_complex;
}
...
{
    for (int iv = 0; iv < phi.rows(); iv++)
        fft.inv(eigen_temp_vec_complex, phi_fft.row(iv)), phi.row(iv) = eigen_temp_vec_complex.real();
}

Eigen::MatrixXcd get_dft_matrix(int nw, double period)
{
    Eigen::MatrixXcd matrix = Eigen::MatrixXcd::Zero(2 * nw + 1, 2 * nw + 1);
    for (int ir = 0; ir < 2 * nw + 1; ir++)
        for (int ic = 0; ic < 2 * nw + 1; ic++)
            matrix(ir, ic) = exp(-1i * 2.0 * M_PI * ((double)ir) * ((double)ic) / ((2 * nw + 1) * period));
    return matrix;
}
...
{
    int nw = (phi.cols() - 1) / 2;
    phi_dft.leftCols(nw + 1) = phi * dft_matrix.leftCols(nw + 1);
    phi_dft.rightCols(nw) = phi_dft.block(0, 1, phi_dft.rows(), nw).conjugate().rowwise().reverse();
}

void calc_idft(Eigen::MatrixXd &phi, Eigen::MatrixXcd &phi_dft, Eigen::MatrixXcd &idft_matrix)
{
    int nw = (idft_matrix.cols() - 1) / 2;
    phi = phi_dft.col(0).real() * idft_matrix.row(0).real() - phi_dft.col(0).imag() * idft_matrix.row(0).imag();
    phi = phi + 2.0 * (phi_dft.block(0, 1, phi_dft.rows(), nw).real() * idft_matrix.block(1, 0, nw, idft_matrix.cols()).real() - phi_dft.block(0, 1, phi_dft.rows(), nw).imag() * idft_matrix.block(1, 0, nw, idft_matrix.cols()).imag());
}

double round_digits(double a, int num_digits)
{
    return (round(a * pow(10.0, num_digits)) / pow(10.0, num_digits));
}

string to_string_precision(double a, int num_digits)
{
    return (to_string(a).substr(0, std::to_string(a).find(".") + num_digits + 1));
}
...
{ // https://stackoverflow.com/a/42978529
    struct stat sb;
    if (stat(path, &sb) == 0 && S_ISDIR(sb.st_mode)) // https://stackoverflow.com/a/24544128
        if (nftw(path, rmFiles, 10, FTW_DEPTH | FTW_MOUNT | FTW_PHYS) < 0)
        {
            perror("ERROR: remove_full_directory");
            exit(1);
        }
}