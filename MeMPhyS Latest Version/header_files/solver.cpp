...
void SOLVER::init(POINTS &points, CLOUD &cloud, PARAMETERS &parameters, vector<bool> &dirichlet_flag, double unsteady_term_coeff, double conv_term_coeff, double diff_term_coeff, bool log_flag)
{
    ...
    transform(solver_type.begin(), solver_type.end(), solver_type.begin(), ::tolower);
    ...
    if (strcmp(solver_type.c_str(), "sor") == 0)
        system_size = points.nv; //no regularization for SOR
    else
    ...
    else if (strcmp(solver_type.c_str(), "sor") == 0)
    {
        EIGEN_set_coeff();
        SOR_rel_res.resize(n_iter);
        SOR_abs_res.resize(n_iter);
    }
    else
    {
...
void SOLVER::SOR(POINTS &points, PARAMETERS &parameters, Eigen::VectorXd &field, Eigen::VectorXd &rhs)
{
    double diag_coeff, phi_new, runtime;
    double rhs_norm = rhs.lpNorm<1>();
    if (rhs_norm < 1E-10)
        rhs_norm = 1.0; //to avoid division by zero
    clock_t clock_t1 = clock(), clock_t2 = clock();
    for (int i1 = 0; i1 < n_iter; i1++)
    {
        for (int ir = 0; ir < coeff_EIGEN.outerSize(); ++ir)
        {
            phi_new = rhs[ir];
            for (Eigen::SparseMatrix<double, Eigen::RowMajor>::InnerIterator iter(coeff_EIGEN, ir); iter; ++iter)
            {
                if (iter.row() == iter.col())
                    diag_coeff = iter.value();
                else
                    phi_new = phi_new - (iter.value() * field[iter.col()]);
            }
            if (fabs(diag_coeff) < 1E-10)
            {
                printf("\n\nSOLVER::SOR diag_coeff at row no.: %i is close to zero: %g\n\n", ir, diag_coeff);
                throw bad_exception();
            }
            phi_new = phi_new / diag_coeff;
            field[ir] = (SOR_omega * phi_new) + ((1.0 - SOR_omega) * field[ir]);
        }
        SOR_abs_res[i1] = (coeff_EIGEN * field.head(points.nv) - rhs.head(points.nv)).lpNorm<1>() / points.nv;
        SOR_rel_res[i1] = SOR_abs_res[i1] / rhs_norm;

        runtime = ((double)(clock() - clock_t2)) / CLOCKS_PER_SEC;
        if (runtime > 5.0)
        {
            printf("        SOLVER::SOR iter: %i, residual ratio: %g\n", i1, SOR_abs_res[i1] / SOR_abs_res[0]);
            printf("        SOLVER::SOR Completed iter: %i of n_iter: %i (%g percent) in CPU time: %.2f seconds\n\n", i1, n_iter, 100 * (i1 + 1.0) / n_iter, ((double)(clock() - clock_t1)) / CLOCKS_PER_SEC);
            clock_t2 = clock();
        }
    }
}
...
void SOLVER::general_solve(POINTS &points, PARAMETERS &parameters, Eigen::VectorXd &field_new, Eigen::VectorXd &field_old, Eigen::VectorXd &rhs)
{
    ...
    {
        field_new = field_old;
        SOR(points, parameters, field_new, rhs);
    }
    else
    {
        cout << "\n\nERROR from SOLVER::SOLVER solver_type should be either hypre_ilu_gmres, eigen_ilu_bicgstab or eigen_direct; current value: " << solver_type << "\n\n";
